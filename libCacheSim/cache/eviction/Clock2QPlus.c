...
// ****                                                               ****
// ***********************************************************************
static void Clock2QPlus_free(cache_t *cache);
static bool Clock2QPlus_get(cache_t *cache, const request_t *req);

static cache_obj_t *Clock2QPlus_find(cache_t *cache, const request_t *req,
                                     const bool update_cache);
static cache_obj_t *Clock2QPlus_insert(cache_t *cache, const request_t *req);
static cache_obj_t *Clock2QPlus_to_evict(cache_t *cache, const request_t *req);
static void Clock2QPlus_evict(cache_t *cache, const request_t *req);
static bool Clock2QPlus_remove(cache_t *cache, const obj_id_t obj_id);
static inline int64_t Clock2QPlus_get_occupied_byte(const cache_t *cache);
static inline int64_t Clock2QPlus_get_n_obj(const cache_t *cache);
static inline bool Clock2QPlus_can_insert(cache_t *cache, const request_t *req);
...
cache_t *Clock2QPlus_init(const common_cache_params_t ccache_params,
                          const char *cache_specific_params) {
  cache_t *cache =
      cache_struct_init("Clock2QPlus", ccache_params, cache_specific_params);
  cache->cache_init = Clock2QPlus_init;
  cache->cache_free = Clock2QPlus_free;
  cache->get = Clock2QPlus_get;
  cache->find = Clock2QPlus_find;
  cache->insert = Clock2QPlus_insert;
  cache->evict = Clock2QPlus_evict;
  cache->remove = Clock2QPlus_remove;
  cache->to_evict = Clock2QPlus_to_evict;
  cache->get_n_obj = Clock2QPlus_get_n_obj;
  cache->get_occupied_byte = Clock2QPlus_get_occupied_byte;
  cache->can_insert = Clock2QPlus_can_insert;

  cache->obj_md_size = 0;

  cache->eviction_params = malloc(sizeof(Clock2QPlus_params_t));
  memset(cache->eviction_params, 0, sizeof(Clock2QPlus_params_t));
  ...
  Clock2QPlus_parse_params(cache, Clock2QPlus_DEFAULT_CACHE_PARAMS);
  if (cache_specific_params != NULL) {
    Clock2QPlus_parse_params(cache, cache_specific_params);
  }
  ...
  snprintf(cache->cache_name, CACHE_NAME_ARRAY_LEN,
           "Clock2QPlus-%.4lf-%d-%.2lf", params->fifo_size_ratio,
...
static cache_obj_t *Clock2QPlus_insert(cache_t *cache, const request_t *req) {
  ...
  if (params->hit_on_ghost) {
    ...
    if (!params->has_evicted && params->fifo->get_occupied_byte(params->fifo) >=
      ...
      obj->Clock2QPlus.insertion_time = params->n_obj_admit_to_fifo;
    }
  }
...
static void Clock2QPlus_evict(cache_t *cache, const request_t *req) {
  ...
  if (main->get_occupied_byte(main) > main->cache_size ||
    ...
    Clock2QPlus_evict_fifo(cache, req);
  }
}
...
static inline bool Clock2QPlus_can_insert(cache_t *cache,
                                          const request_t *req) {
  Clock2QPlus_params_t *params = (Clock2QPlus_params_t *)cache->eviction_params;
...
static void Clock2QPlus_parse_params(cache_t *cache,
                                     const char *cache_specific_params) {
  Clock2QPlus_params_t *params =
      (Clock2QPlus_params_t *)(cache->eviction_params);
