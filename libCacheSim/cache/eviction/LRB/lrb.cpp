...
void LRBCache::train() {
  ...
  LGBM_DatasetCreateFromCSR(
      static_cast<void *>(training_data->indptr.data()), C_API_DTYPE_INT32,
      training_data->indices.data(),
      static_cast<void *>(training_data->data.data()), C_API_DTYPE_FLOAT64,
      training_data->indptr.size(), training_data->data.size(),
  ...
  // init booster
  LGBM_BoosterCreate(trainData, map_to_string(training_params).c_str(),
                     &booster);
  // train
  ...
  LGBM_BoosterPredictForCSR(
      booster, static_cast<void *>(training_data->indptr.data()),
  ...
  training_time =
      0.95 * training_time + 0.05 * chrono::duration_cast<chrono::milliseconds>(
                                        chrono::system_clock::now() - timeBegin)
                                        .count();
}
...
void LRBCache::sample() {
  ...
  bernoulli_distribution distribution_from_in(static_cast<double>(n_in) /
                                              (n_in + n_out));
...
void LRBCache::update_stat_periodic() {
  ...
  if (0 == obj_distribution[0] && 0 == obj_distribution[1]) {
    ...
    percent_beyond = static_cast<float>(obj_distribution[1]) /
                     (obj_distribution[0] + obj_distribution[1]);
  }
  ...
  if (0 == training_data_distribution[0] &&
      0 == training_data_distribution[1]) {
    positive_example_ratio = 0;
  } else {
    positive_example_ratio =
        static_cast<float>(training_data_distribution[1]) /
...
bool LRBCache::lookup(const SimpleRequest &req) {
  ...
  // first update the metadata: insert/update, which can trigger pending
  // data.mature
  ...
  if (it != key_map.end()) {
    ...
    Meta &meta =
        list_idx ? out_cache_metas[list_pos] : in_cache_metas[list_pos];
    // update past timestamps
    assert(static_cast<int64_t>(meta._key) == req.id);
    ...
    // make this update after update training, otherwise the last timestamp will
    // change
    meta.update(current_seq, n_extra_fields, max_hash_edc_idx, edc_windows,
                hash_edc);
    if (list_idx) {
...
void LRBCache::forget() {
  /*
   * forget happens exactly after the beginning of each time, without doing any
   * other operations. For example, an object is request at time 0 with memory
   * window = 5, and will be forgotten exactly at the start of time 5.
   * */
...
void LRBCache::admit(const SimpleRequest &req) {
  ...
  if (it == key_map.end()) {
    // fresh insert
    key_map.insert(
        {static_cast<uint64_t>(req.id), {0, (uint32_t)in_cache_metas.size()}});
    auto lru_it = in_cache_lru_queue.request(req.id);
    in_cache_metas.emplace_back(req.id, req.size, current_seq,
                                req.extra_features, lru_it);
    _currentSize += size;
  } else {
    ...
    if (it->second.list_pos != tail1_pos) {
      ...
      key_map.find(out_cache_metas[tail1_pos]._key)->second.list_pos =
          it->second.list_pos;
    }
...
pair<uint64_t, uint32_t> LRBCache::rank() {
  ...
  while (idx_row != sample_rate) {
    ...
    for (uint8_t k = 0; k < n_edc_feature; ++k) {
      ...
      uint32_t _distance_idx =
          min(uint32_t(current_seq - meta._past_timestamp) / edc_windows[k],
              max_hash_edc_idx);
      if (meta._extra)
  ...
  if (!(current_seq % 10000))
    inference_time = 0.95 * inference_time +
                     0.05 * chrono::duration_cast<chrono::microseconds>(
                                chrono::system_clock::now() - timeBegin)
                                .count();
  for (int32_t i = sample_rate - n_new_sample; i < (int32_t)sample_rate; ++i) {
  ...
  if (objective == object_miss_ratio) {
    for (uint i = 0; i < sample_rate; ++i) scores[i] *= sizes[i];
  }
  ...
  if (memory_window <= current_seq - meta._past_timestamp) {
    ...
    if (!meta._sample_times.empty()) {
      ...
      uint32_t future_distance =
          current_seq - meta._past_timestamp + memory_window;
      for (auto &sample_time : meta._sample_times) {
        // don't use label within the first forget window because the data is
        // not static
        training_data->emplace_back(meta, sample_time, future_distance,
    ...
    if (old_pos != activate_tail_idx) {
      ...
      key_map.find(in_cache_metas[activate_tail_idx]._key)->second.list_pos =
          old_pos;
    }
    ...
    negative_candidate_queue->insert(
        {meta._past_timestamp % memory_window, meta._key});
...
void LRBCache::remove_from_outcache_metas(Meta &meta, unsigned int &pos,
                                          const uint64_t &key) {
